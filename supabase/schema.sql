-- PROKICKS PLAY · SCHEMA AISLADO · SPRINT 1.1
-- Ejecutar en Supabase SQL Editor. Todas las tablas llevan prefijo prokicks_.
...
create table if not exists public.prokicks_devices (
  id uuid primary key default uuid_generate_v4(),
  spot_id uuid references public.prokicks_spots(id) on delete cascade,
  name text not null,
...
create table if not exists public.prokicks_challenges (
  id uuid primary key default uuid_generate_v4(),
  title text not null,
  creator_id uuid references public.prokicks_profiles(id) on delete set null,
  spot_id uuid references public.prokicks_spots(id) on delete cascade,
...
create table if not exists public.prokicks_challenge_participants (
  id uuid primary key default uuid_generate_v4(),
  challenge_id uuid references public.prokicks_challenges(id) on delete cascade,
...
create table if not exists public.prokicks_results (
  id uuid primary key default uuid_generate_v4(),
  challenge_id uuid references public.prokicks_challenges(id) on delete cascade,
...
alter table public.prokicks_devices enable row level security;
alter table public.prokicks_challenges enable row level security;
alter table public.prokicks_challenge_participants enable row level security;
alter table public.prokicks_results enable row level security;

drop policy if exists "prokicks_profiles_select" on public.prokicks_profiles;
create policy "prokicks_profiles_select" on public.prokicks_profiles for select using (true);
drop policy if exists "prokicks_profiles_insert_own" on public.prokicks_profiles;
create policy "prokicks_profiles_insert_own" on public.prokicks_profiles for insert with check (auth.uid() = id);
drop policy if exists "prokicks_profiles_update_own" on public.prokicks_profiles;
create policy "prokicks_profiles_update_own" on public.prokicks_profiles for update using (auth.uid() = id);
drop policy if exists "prokicks_spots_select" on public.prokicks_spots;
create policy "prokicks_spots_select" on public.prokicks_spots for select using (true);
drop policy if exists "prokicks_devices_select" on public.prokicks_devices;
create policy "prokicks_devices_select" on public.prokicks_devices for select using (true);
drop policy if exists "prokicks_challenges_select" on public.prokicks_challenges;
create policy "prokicks_challenges_select" on public.prokicks_challenges for select using (true);
drop policy if exists "prokicks_challenges_insert_auth" on public.prokicks_challenges;
create policy "prokicks_challenges_insert_auth" on public.prokicks_challenges for insert with check (auth.uid() is not null);
drop policy if exists "prokicks_participants_select" on public.prokicks_challenge_participants;
create policy "prokicks_participants_select" on public.prokicks_challenge_participants for select using (true);
drop policy if exists "prokicks_participants_insert_auth" on public.prokicks_challenge_participants;
create policy "prokicks_participants_insert_auth" on public.prokicks_challenge_participants for insert with check (auth.uid() is not null);
drop policy if exists "prokicks_results_select" on public.prokicks_results;
create policy "prokicks_results_select" on public.prokicks_results for select using (true);
drop policy if exists "prokicks_results_insert_auth" on public.prokicks_results;
create policy "prokicks_results_insert_auth" on public.prokicks_results for insert with check (auth.uid() is not null);
