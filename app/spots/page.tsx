  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const clean = (data || []).map((spot, index) => normalizeSpot(spot as Spot, index)).filter(Boolean) as Spot[];
        ...
        {items.map((raw, index) => {
          ...
          return (
            <article className="card spot-card" key={s.id}>
              <iframe className="map-embed" src={mapEmbedUrl(s.address || s.name)} loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`Mapa ${s.name}`} />
              <div className="row"><h3 className="card-title">{s.name}</h3><span className="tag tag-blue">Activo</span></div>
              ...
              <div className="grid-2">
                ...
                <Link className="btn btn-warm" href={s.maps_url || mapsSearchUrl(s.address || s.name)} target="_blank">Abrir en Google Maps</Link>
              </div>
            </article>
          );
        })}
